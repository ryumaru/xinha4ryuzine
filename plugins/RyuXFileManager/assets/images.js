 ...
 * Package: RyuXFileManager (EFM 1.1.3)
...
{
    showMessage('Loading');
    var mode=window.parent.document.getElementById('manager_mode').value;
    var selection = window.parent.document.getElementById('viewtype');
...
function renameFile(oldPath) {
 // strip directory and extension
  ...
  var rename = function (newName)
  {
    ...
    selection = window.parent.document.getElementById('viewtype');
    var viewtype = selection.options[selection.selectedIndex].value;
...
function copyFile(file,action)
{
	var selection = window.parent.document.getElementById('dirPath');
    var dir = selection.options[selection.selectedIndex].value;
	window.parent.pasteButton({'dir':dir,'file':file,'action':action+'File'});
}
function copyDir(dirToCopy,action)
{
	var selection = window.parent.document.getElementById('dirPath');
    var dir = selection.options[selection.selectedIndex].value;
	window.parent.pasteButton({'dir':dir,'file':dirToCopy,'action':action+'Dir'});
}
...
function showPreview(f_url)
{
    ...
    f_url ? window.parent._backend_url + '__function=thumbs&img=' + f_url :window.parent.opener.Xinha.getPluginDir('RyuXFileManager')+'/img/1x1_transparent.gif';
}
