...
function __xinha_dlg_init( win_dim ) {
  ...
  if (!window.opener) {
  	var window_editor_skin = window.parent._editor_skin;
  } else {
  	var window_editor_skin = window.opener._editor_skin;
  }
  if(window_editor_skin) {
    var head = document.getElementsByTagName("head")[0];
    var link = document.createElement("link");
    link.type = "text/css";
    link.href = window.opener._editor_url + 'skins/' + window_editor_skin + '/skin.css';
    link.rel = "stylesheet";
...
function __dlg_close(val) {
	if (!opener) {
	  window.parent.Dialog._return(val);
	} else {
