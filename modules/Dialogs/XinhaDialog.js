...
// RYU - icon not needed when integrated to Ryuzine Writer
/*  
...
//  Xinha.freeLater(this, "icon");	// RYU - not needed
...
/*	this is supposed to scroll the window? But it can't because window doesn't
	scroll in Ryuzine Writer, all elements are absolute or fixed 100%x100% 
	also kicks error if you hit ESC key before any dialogs have opened - RYU
...
Xinha.Dialog.prototype.dragIt = function(ev)
{
  ...
  {
    posY = (ev.clientY + this.scrollPos.y) - dialog.yOffs;
    posX = (ev.clientX + this.scrollPos.x) - dialog.xOffs;
    if ( posY < 0 || posY > viewport.y-100 || posX < 0-(dialog.xOffs*.90) || posX > viewport.x-(dialog.xOffs) ) {
    	// prevent dialog being dragged off screen -- RYU MARU
    } else {
    newPos = {top: posY+"px",left: posX+"px"};
    dialog.posDialog(newPos);
    }
  }
  else if (dialog.rootElem.style.position == 'fixed')
  {
    posY = ev.clientY  - dialog.yOffs;
    posX = ev.clientX - dialog.xOffs;
 ...
 // Generates errors if not inside if/else above -- RYU MARU
//  dialog.posDialog(newPos);
};
/** Ends dragging
 * @private
...
Xinha.Dialog.prototype.resizeIt = function(ev)
{
  ...
  var newSize = {};
  newSize.width  = (( posX > 10) ? posX : 10) + 8;
  newSize.height = (( posY > 10) ? posY : 10);
  // prevent panels from being resized larger than workspace	-- RYU MARU
  if (	newSize.height > document.getElementById('editor').offsetHeight-50 ||
  		newSize.width > document.getElementById('editor').offsetWidth-50 ) {
  } else {
  	newSize.width = newSize.width+"px";
  	newSize.height = newSize.height+"px";
...
//  this.icon.style.display = 'none';	// RYU - not needed
  ...
  else
  {
	...
	// Add back "panel" and remove "floating" so we can style as docked
  Xinha.addClasses(rootElem, 'panel');
  Xinha.removeClasses(rootElem,'floating');
...
//  dialog.icon.style.display = '';	// RYU - not needed
  ...
  if (dialog.size.width) 
  {
	...
	// Remove "panel" and add "floating" so we can style as detached
  Xinha.removeClasses(rootElem, 'panel');
  Xinha.addClasses(rootElem, 'floating');
  editor.removePanel(rootElem);
//  document.body.appendChild(rootElem);
// Ryuzine Writer needs panels to travel with editor, not window
  document.getElementById('editor').appendChild(rootElem);
