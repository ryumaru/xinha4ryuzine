/**
 * Functions for the RyuXFileManager, used by manager.php only
...
//Translation
function i18n(str) {
    return Xinha._lc(str, 'RyuXFileManager');
}
...
//    var editor = window.dialogArguments.editor;
	  var editor = window.parent.Xinha.editor;
...
function pasteButton(action)
{
	...
	if (!buttonHolder.firstChild)
	{
		...
		var img = document.createElement('img');
			img.src = window.opener.Xinha.getPluginDir('RyuXFileManager')+'/img/edit_paste.gif';
			img.alt = i18n('Paste');
			a.appendChild(img);
...
//              param[id] = makeURL(base_url,el.value);
			// To ensure Ryuzine is portable relative URL only //
				param[id] = 'images'+el.value;
            } else {
                param[id] = el.value;
            }
        }
        ...
        {
          // Yup, need to resize
          var resized = Xinha._geturlcontent(window.opener.Xinha.getPluginDir('RyuXFileManager') + '/' + _backend_url + '&__function=resizer&img=' + encodeURIComponent(document.getElementById('f_url').value) + '&width=' + param.f_width + '&height=' + param.f_height);
