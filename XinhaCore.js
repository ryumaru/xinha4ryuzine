// WARNING: customized for use with Ryuzine Writer, do not overwrite or replace!
 ...
 * @type feature check
 */
Xinha.is_ie11  = (!(window.ActiveXObject) && "ActiveXObject" in window);
...
/** Version Number, if browser is Microsoft Internet Explorer 11+
 * UA string has no "msie" so version will report as NaN
 * @type force
 */
if (Xinha.is_ie11) { Xinha.ie_version = 11; }
/** Browser is Opera
 * @type Boolean 
 */
 ...
 * IE 11 pretends it is Gecko, ActiveXObject check detects for it
 * @type Boolean 
 */
Xinha.is_real_gecko = (navigator.product == "Gecko" && !Xinha.is_webkit && !("ActiveXObject" in window) );
...
// RYU Edit: prevents warning dialog if WYSIWYG is disabled //
if ( Xinha.isRunLocally && Xinha.isSupportedBrowser && com.kmhcreative.RyuzineWriter.config.wysiwyg == 1)
{
  alert('The Rich Text Editor *must* be run from a HTTP web server. Locally opened files (those that use the "file://" protocol) cannot properly function. Ryuzine Writer will try to initialize but may not be correctly loaded.');
}
 ...
 *    });</pre>
 *
 *	NOTE: if "id" is defined it is also used for the button's element.id in the DOM, however
 *	The none of the default buttons you set in Xinha.config.toolbar will have a unique ID.
 *																					- RYU
 *
 */
Xinha.Config.prototype.registerButton = function(id, tooltip, image, textMode, action, context)
{
  if ( typeof id == "string" )
  {
    this.btnList[id] = [ tooltip, image, textMode, action, context, id ];
  }
  else if ( typeof id == "object" )
  {
    this.btnList[id.id] = [ id.tooltip, id.image, id.textMode, id.action, id.context, id.id ];
  }
  else
  {
...
Xinha.prototype._createToolbar1 = function (editor, toolbar, tb_objects)
{
  ...
  function createSelect(txt)
  {
    ...
    if ( options )
    {
	  dv = document.createElement("div");	// RYU - style-able container for <select>
	  dv.className = "droplist "+txt;		// gives specific reference to container
      ...
      dv.appendChild(el);	// RYU - put <select> inside <div>
    }
    return dv;	// RYU - return <DIV> instead of just the <select> element
  ...
  function createButton(txt)
  {
    ...
    switch (txt)
    {
      ...
      case "textindicator":
        el = document.createElement("div");
        el.appendChild(document.createTextNode("Current Style"));
    ...
    if ( !el && btn )
    {
      ...
      if (btn[5]!=undefined) { el.id=btn[5]; }	// (object[id] -> element[id]) - RYU
...
Xinha.prototype.sizeEditor = function(width, height, includingBars, includingPanels)
{
  ...
  if (this._iframe) { // Firefox seems to dislike this onload - RYU
 ...
 */
Xinha.prototype.setMode = function(mode,external)
{
	if ( external != null ) {
		var thiseditor = __xinhas[0];
	} else {
		var thiseditor = this;
	}
  var html;
  ...
  {
    mode = thiseditor._editMode == "textmode" ? "wysiwyg" : "textmode";
  }
  switch ( mode )
  {
    case "textmode":
      thiseditor.firePluginEvent('onBeforeMode', 'textmode');
      thiseditor._toolbarObjects.htmlmode.swapImage(thiseditor.config.iconList.wysiwygmode); 
      thiseditor.setCC("iframe");
      html = thiseditor.outwardHtml(thiseditor.getHTML());
      thiseditor.setHTML(html);

      // Hide the iframe
      thiseditor.deactivateEditor();
      thiseditor._iframe.style.display   = 'none';
      thiseditor._textArea.style.display = '';
      
      // Add htmlmode class
      Xinha._addClass(document.getElementById('leftbox').getElementsByTagName('table')[0],"htmlmode");

//	 document.getElementById('ryu_picker').style.display="none"; // Hide RYUPicker Panel
	
/*	 var buttons = document.getElementById('leftbox').getElementsByTagName('table')[0].getElementsByTagName('div')[0].getElementsByTagName('table');
...
*/	 		
      ...
      if ( thiseditor.config.statusBar )
      {
        thiseditor._statusBarTree.style.display = "none";
        thiseditor._statusBarTextMode.style.display = "";
      }
      thiseditor.findCC("textarea");
      thiseditor.notifyOf('modechange', {'mode':'text'});
      thiseditor.firePluginEvent('onMode', 'textmode');
    break;

    case "wysiwyg":
      thiseditor.firePluginEvent('onBeforeMode', 'wysiwyg');
      thiseditor._toolbarObjects.htmlmode.swapImage([thiseditor.imgURL('images/ed_buttons_main.png'),7,0]); 
      thiseditor.setCC("textarea");
      html = thiseditor.inwardHtml(thiseditor.getHTML());
      thiseditor.deactivateEditor();
      thiseditor.setHTML(html);
      thiseditor._iframe.style.display   = '';
      thiseditor._textArea.style.display = "none";
      thiseditor.activateEditor();
      
//      document.getElementById('ryu_picker').style.display="block"; // Show RYUPicker Panel	
/*      
     var buttons = document.getElementById('leftbox').getElementsByTagName('table')[0].getElementsByTagName('div')[0].getElementsByTagName('table');
	 var buttonlist = buttons.length;
	 for (var x = 0; x < buttonlist; x++) {
...
*/    
      if ( thiseditor.config.statusBar )
      {
        thiseditor._statusBarTree.style.display = "";
        thiseditor._statusBarTextMode.style.display = "none";
      }
      thiseditor.findCC("iframe");
      thiseditor.notifyOf('modechange', {'mode':'wysiwyg'});
      thiseditor.firePluginEvent('onMode', 'wysiwyg');

	  Xinha._removeClass(document.getElementById('leftbox').getElementsByTagName('table')[0],"htmlmode");
 ...
 *
 *	RYU MARU EDITS 7/2014
 *	string split didn't give character for split
 */
 ...
 *  @param {String}   className The class to be removed
 *
 *	RYU MARU Edit 7/2014
 *	string splits didn't give character for split
 *  returned new string but did not apply to element
 */
...
//  return new_classes.join(' '); // all this did was return the list?  fixed to apply
  el.className = new_classes.join(' ').trim();
};
