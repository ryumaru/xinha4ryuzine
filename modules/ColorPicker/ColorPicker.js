  ...
  Xinha.colorPicker = function (params)
  {
    ...
    this.table.style.zIndex = params.zindex || '1050';	// RYU - allows control of picker stacking order
    ...
    this.open = function(anchorage,element,initcolor)
    {
	  ...
	  this.element = element;	// RYU - pass caller element into object
      this.pick_color();
...
//          unpickCell(valuerow,valuecol);
  ...
  //          unpickCell(pickrow,pickcol);
   ...
   //           this.style.borderColor = picker.saved_cells[this.thisrow][this.thiscol].colorCode;
        ...
        but.style.cursor = 'pointer';
        // RYU - next line added picker.element.style.backgroundColor = picker.colorChoice.value to change caller color
        // similar to the InputBinding below, but does not create the element, just styles whatever invoked picker on OK
        but.onclick = function() { Xinha.colorPicker.remember(picker.chosenColor.value, picker.savecolors); picker.callback(picker.chosenColor.value); picker.element.style.backgroundColor=picker.chosenColor.value; picker.close(); };
   ...
   //       div.onmouseover = function()
    ...
    this.close = function()
    {
      ...
      document.body.removeChild(this.table);	// RYU - prevents every invocation creating a new picker
    };
