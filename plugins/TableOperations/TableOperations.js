...
//
// Modified by K.M Hansen for use with Ryuzine - 07 April 2015
...
function TableOperations(editor) {
  ...
  // Add a toggle button to show/hide Table Operations - RYU
   cfg.registerButton({
   		id		 : "tableoptoggle",
   		tooltip	 : Xinha._lc("Toggle Table Operations","TableOperations"),
    	image    : editor.imgURL("toggle-table-ops.gif","TableOperations"),
   		textMode : false,
   		action	 : function(){ RYU.toggleTableOptions();}	// function is in writer.js - RYU
   	});
   	cfg.toolbar[2].push("tableoptoggle");
  ...
  for (var i = 0; i < bl.length; ++i) {
    var btn = bl[i];
    if (!btn) {
 //     if(cfg.TableOperations.showButtons) toolbar.push("separator");	// no separators! - RYU
    } else {
      var id = "TO-" + btn[0];
