...
/**
 * !IMPORTANT!
 * This is not the original configuration file, it has been edited
 * for use with Ryuzine Writer and is also backed up as "ryu.config.inc.php"
 * in case you update Extended File Manager and overwrite this file
 *
 * RyuXFileManager configuration file.
 * Author: K.M. Hansen
 * Version: 1.0
 * Package: RyuXFileManager
 * http://www.ryumaru.com
 
 * Based on work by: Wei Zhuo, Afru
...
*
* The ?__plugin=RyuXFileManager& is required.
*/

$IMConfig['backend_url'] = "backend.php?__plugin=RyuXFileManager&";
...
$MY_PATH = $MY_PATH[0].'images/';
$IMConfig['images_dir'] = $MY_PATH;
...
$MY_URL = explode('ryuzinewriter',$MY_URL);
$MY_URL = $MY_URL[0].'images/';
$IMConfig['images_url']= $MY_URL;
//$IMConfig['images_url'] = str_replace( array("backend.php","manager.php"), "", $_SERVER["PHP_SELF"] ) . $IMConfig['/images'];
...
$IMConfig['allowed_image_extensions'] = array("jpg","jpeg","gif","png","bmp");
$IMConfig['allowed_link_extensions'] = array("jpg","jpeg","gif","js","php","pdf","zip","txt","psd","png","html","swf","xml","xls","doc");
...
*/
$IMConfig['thumbnail_extensions'] = array("jpg", "jpeg", "gif", "png", "bmp");

/*
  Thumbnail dimensions.
*/
$IMConfig['thumbnail_width'] = 75;
...
require_once(realpath(dirname(__FILE__) . '/../../php/php-xinha.php'));
...
elseif(isset($_REQUEST['backend_config']))
{
  ...
  if($_REQUEST['backend_config_secret_key_location'] !== 'Xinha:RyuXFileManager')
  {
    trigger_error('Programming Error - please contact the website administrator/programmer to alert them to this problem. A non-default backend key location is being used to pass backend data to Xinha, but the same key location is not being used to receive data.  The special backend configuration has been ignored.  To resolve this, you should edit plugins/RyuXFileManager/config.php and change the default key location from "Xinha:RyuXFileManager" to your desired non default.  See: http://trac.xinha.org/ticket/1518', E_USER_ERROR);    
  }
  else
  {
