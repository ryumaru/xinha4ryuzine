/**
 * RyuXFileManager extended-file-manager.js file.
 ...
 * Package: RyuXFileManager 1.2 (EFM 1.4)
 */
...
function RyuXFileManager(editor)
{
    ...
    if (cfg.RyuXFileManager.use_linker) {
        cfg.registerButton({
            id        : "linkfile",
            tooltip   : Xinha._lc("Insert File Link",'RyuXFileManager'),
            image     : Xinha.getPluginDir('RyuXFileManager') + '/img/ed_linkfile.gif',
            textMode  : false,
    ...
    var manager = editor.config.RyuXFileManager.backend + '__function=manager';
    if(cfg.RyuXFileManager.backend_config != null)
    {
      manager += '&backend_config='
        + encodeURIComponent(cfg.RyuXFileManager.backend_config);
      manager += '&backend_config_hash='
        + encodeURIComponent(cfg.RyuXFileManager.backend_config_hash);
      manager += '&backend_config_secret_key_location='
        + encodeURIComponent(cfg.RyuXFileManager.backend_config_secret_key_location);
    }

    if(cfg.RyuXFileManager.backend_data != null)
    {
        for ( var i in cfg.RyuXFileManager.backend_data )
        {
            manager += '&' + i + '=' + encodeURIComponent(cfg.RyuXFileManager.backend_data[i]);
        }
    }
    cfg.RyuXFileManager.manager = manager;
}

RyuXFileManager._pluginInfo = {
    name          : "RyuXFileManager",
    version       : "1.1",
...
Xinha.Config.prototype.RyuXFileManager =
{
  'use_linker': false,
  'backend'    : Xinha.getPluginDir('RyuXFileManager') + '/backend.php?__plugin=RyuXFileManager&',
  'backend_data' : null,
  // deprecated keys, use passing data through e.g. xinha_pass_to_php_backend()
  'backend_config'     : null,
  'backend_config_hash': null,
  'backend_config_secret_key_location': 'Xinha:RyuXFileManager'
};
...
//    Dialog(this.config.RyuXFileManager.manager, function(param){
 	  Dialog(null, function(param){
        if (!param)
    ...
    RYU.toggleDialog('file_manager');
    document.getElementById('fileManager').src = "ryuzinewriter/xinha4ryuzine/plugins/RyuXFileManager/backend.php?__plugin=RyuXFileManager&__function=manager";
};
}
...
Xinha.prototype._linkFile = function(link) {
    ...
    if (!link) {
        ...
        if (compare == 0) {
            alert(Xinha._lc("You must select some text before making a new link.", 'RyuXFileManager'));
            return;
        }
        outparam.param = {
            f_href : '',
...
//    Dialog(this.config.RyuXFileManager.manager+'&mode=link', function(param){
    Dialog(null, function(param){
        if (!param)
            return false;
        var a = link;
