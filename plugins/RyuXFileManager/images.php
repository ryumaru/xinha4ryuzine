<?php
/**
 * RyuXFileManager images.php file. Shows folders and files.
    ...
    switch ($afruViewType) {
        case 'listview':
    		...
    		foreach($list[1] as $entry => $file)
    		{
                ...
                <tr>
        		  <td>
					<a href="#" class="thumb" style="cursor: pointer;vertical-align:middle;" ondblclick="this.onclick();window.parent.onOK();" onclick="selectImage('<?php echo $file['relative'];?>', '<?php echo preg_replace('#\..{3,4}$#', '', $entry); ?>', <?php echo $file['image'][0];?>, <?php echo $file['image'][1]; ?>);return false;" title="<?php echo $entry; ?> - <?php echo Files::formatSize($file['stat']['size']); ?>" <?php if ($insertMode == 'image') { ?> onmouseover="showPreview('<?php echo $file['relative'];?>')" onmouseout="showPreview(window.parent.document.getElementById('f_url').value)" <?php } ?> >
        ...
        <?php }  ?>
    		...
    		foreach($list[1] as $entry => $file)
    		{
                ...
                <div class="thumb_holder" id="holder_<?php echo asc2hex($entry) ?>">
                  <a href="#" class="thumb" style="cursor: pointer;" ondblclick="this.onclick();window.parent.onOK();" onclick="selectImage('<?php echo $file['relative'];?>', '<?php echo preg_replace('#\..{3,4}$#', '', $entry); ?>', <?php echo $file['image'][0];?>, <?php echo $file['image'][1]; ?>);return false;" title="<?php echo $entry; ?> - <?php echo Files::formatSize($file['stat']['size']); ?>">
...
/*<![CDATA[*/
	...
	init = function()
	{
        __dlg_translate('RyuXFileManager');
...
//if(window.parent.document.getElementById('manager_mode').value=="image")
//emptyProperties();
